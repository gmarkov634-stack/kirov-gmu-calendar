...
export function buildMedicinePublicationPlan({ manifest, source, evidence, qa }) {
  ...
  if (digestNormalizedEvents(events) !== candidateDigest) throw new Error('expanded events do not match QA candidate digest');
  ...
  const versions = source.expectedGroupIds.map((groupId) => Object.freeze({
    groupId,
    versionId: stableVersionId({ groupId, candidateDigest }),
