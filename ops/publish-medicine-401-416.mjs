...
const APPROVED_EVENT_COUNT = 2310;
...
if (source.source?.sha256 !== APPROVED_SOURCE_SHA256 || manifest.sourceSha256 !== APPROVED_SOURCE_SHA256) {
  throw new Error('medicine 401-416 official source SHA-256 does not match approved source');
}
if (qa.decision !== 'pass' || qa.candidateDigest !== APPROVED_CANDIDATE_DIGEST) {
  throw new Error('medicine 401-416 QA gate does not match approved PASS candidate');
}
if (qa.checks?.some((check) => check?.status === 'fail')) {
  throw new Error('medicine 401-416 QA contains a failing check');
}
if (evidence.source?.sha256 !== APPROVED_SOURCE_SHA256) {
  throw new Error('medicine 401-416 evidence source SHA-256 mismatch');
}
if (evidence.candidate?.candidateDigest !== APPROVED_CANDIDATE_DIGEST) {
  throw new Error('medicine 401-416 evidence candidate digest mismatch');
}
if (evidence.candidate?.eventCount !== APPROVED_EVENT_COUNT) {
  throw new Error('medicine 401-416 evidence event count mismatch');
}
if (JSON.stringify(source.expectedGroupIds) !== JSON.stringify(GROUPS) || JSON.stringify(manifest.groupTable) !== JSON.stringify(GROUPS)) {
  throw new Error('medicine 401-416 group scope must be exactly 401-416');
}
...
if (candidateDigest !== APPROVED_CANDIDATE_DIGEST) {
  throw new Error(`expanded medicine 401-416 candidate digest mismatch: ${candidateDigest}`);
}
if (events.length !== APPROVED_EVENT_COUNT) {
  throw new Error(`expanded medicine 401-416 event count mismatch: ${events.length}`);
}
...
const versions = GROUPS.map((groupId) => ({
  ...
  eventCount: EXPECTED_GROUP_EVENT_COUNTS[groupId]
}));
const parsingResult = {
