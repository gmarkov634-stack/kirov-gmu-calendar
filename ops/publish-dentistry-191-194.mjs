...
async function loadPlan() {
  ...
  if (source.universityId !== 'kirov-gmu' || source.programId !== 'dentistry' || source.course !== 1) throw new Error('unexpected Dentistry course-1 source identity');
  ...
  if (draft.sourceSha256 !== sourceSha256 || publication.sourceSha256 !== sourceSha256) throw new Error('Dentistry course-1 source SHA-256 evidence mismatch');
  ...
  if (qa.decision !== 'pass' || qa.readyForScheduleVersion !== true || qa.unresolvedSemanticItemCount !== 0) throw new Error('Dentistry course-1 QA is not ScheduleVersion-ready');
  ...
  if (candidateDigest !== qa.candidateDigest || candidateDigest !== publication.candidateDigest) throw new Error(`Dentistry course-1 approved artifact digest mismatch: ${candidateDigest}`);
  const normalizedEventSetDigest = digestNormalizedEvents(events);
  if (normalizedEventSetDigest !== publication.eventSetDigest) throw new Error(`Dentistry course-1 event-set digest mismatch: ${normalizedEventSetDigest}`);
  if (events.length !== 1656 || events.length !== qa.eventCount || events.length !== publication.eventCount) throw new Error(`Dentistry course-1 event count mismatch: ${events.length}`);
  if (events.some((event) => event.timeSemantics !== 'floating')) throw new Error('Dentistry course-1 publication candidate must contain floating events only');
  if (new Set(events.map((event) => event.eventId)).size !== events.length) throw new Error('Dentistry course-1 duplicate eventId detected');
  ...
  if (JSON.stringify([...actualFacultativeIds].sort()) !== JSON.stringify([...facultativeIds].sort())) throw new Error('Dentistry course-1 facultative catalog does not match publication evidence');

  for (const groupId of expectedGroups) {
    if (actualCounts[groupId] !== publication.groupEventCounts?.[groupId] || actualCounts[groupId] !== qa.eventCountByGroup?.[groupId]) throw new Error(`group ${groupId} event count evidence mismatch`);
...
try {
  ...
  for (const version of plan.versions) {
    ...
    if (countVevents(defaultIcs) !== expectedDefaultCount) throw new Error(`group ${version.groupId} default-off facultative ICS count verification failed`);

    const allFacultativeChoices = Object.fromEntries(plan.publication.facultativeIds.map((facultativeId) => [facultativeId, true]));
    const allFacultativesIcs = unfoldIcs(core.renderPublishedScheduleIcs({ scheduleVersion: published.scheduleVersion, events: published.events, calendarName: `КГМУ стоматология ${version.groupId}`, preferences: { facultativeChoices: allFacultativeChoices } }));
    if (countVevents(allFacultativesIcs) !== version.eventCount) throw new Error(`group ${version.groupId} all-facultatives ICS VEVENT count verification failed`);
    if (published.events.some((event) => event.assessment) && !allFacultativesIcs.includes('DESCRIPTION:')) throw new Error(`group ${version.groupId} assessment metadata is missing from rendered ICS`);
  }
