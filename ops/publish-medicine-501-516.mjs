...
const APPROVED_GROUP_EVENT_COUNT = 150;
...
const SELECTION_OPTIONS = Object.freeze(['stream-1', 'stream-2']);
const PE_EVENTS_PER_OPTION_PER_GROUP = 16;
const BASE_EVENTS_PER_GROUP = APPROVED_GROUP_EVENT_COUNT - (PE_EVENTS_PER_OPTION_PER_GROUP * SELECTION_OPTIONS.length);
const PERSONALIZED_EVENTS_PER_GROUP = BASE_EVENTS_PER_GROUP + PE_EVENTS_PER_OPTION_PER_GROUP;
...
function selectionEvents(events, optionId = null) {
  return events.filter((event) =>
    event.selection?.selectionGroupId === SELECTION_GROUP_ID &&
    (optionId == null || event.selection?.selectionOptionId === optionId)
  );
}

async function verifyCoreBoundary(coreRoot, coreEvidence) {
  const [schema, renderer, preferences] = await Promise.all([
    readFile(resolve(coreRoot, 'contracts/normalized-event.schema.json')),
    readFile(resolve(coreRoot, 'src/calendar/ics-renderer.js')),
    readFile(resolve(coreRoot, 'src/calendar/preferences.js'))
  ]);
  const schemaBlob = gitBlobSha(schema);
  const rendererBlob = gitBlobSha(renderer);
  const preferencesBlob = gitBlobSha(preferences);
  ...
  if (preferencesBlob !== coreEvidence.preferencesBlob) {
    throw new Error(`deployed core calendar preferences blob mismatch: ${preferencesBlob}`);
  }
  return { schemaBlob, rendererBlob, preferencesBlob };
}
...
if (evidence.sourceSha256 !== APPROVED_SOURCE_SHA256) {
  throw new Error('medicine 501-516 evidence source SHA-256 mismatch');
}
if (evidence.candidateDigest !== APPROVED_CANDIDATE_DIGEST) {
  throw new Error('medicine 501-516 evidence candidate digest mismatch');
}
if (evidence.eventCount !== APPROVED_EVENT_COUNT) {
...
if (evidence.selectionGroupId !== SELECTION_GROUP_ID || JSON.stringify(evidence.selectionOptionIds) !== JSON.stringify(SELECTION_OPTIONS)) {
  throw new Error('medicine 501-516 PE selection metadata does not match approved personalization model');
}
if (evidence.unresolvedAmbiguities !== 0 || evidence.duplicateEvents !== 0 || evidence.datesOutsideSourceCalendar !== 0) {
  throw new Error('medicine 501-516 evidence contains unresolved QA defects');
}
...
for (const groupId of GROUPS) {
  ...
  groupEventCounts[groupId] = groupEvents.length;
  const optionCounts = Object.fromEntries(SELECTION_OPTIONS.map((optionId) => [
    optionId,
    selectionEvents(groupEvents, optionId).length
  ]));
  groupSelectionCounts[groupId] = optionCounts;

  if (groupEvents.length !== APPROVED_GROUP_EVENT_COUNT || evidence.groupEventCounts?.[groupId] !== APPROVED_GROUP_EVENT_COUNT) {
    throw new Error(`group ${groupId} must contain exactly ${APPROVED_GROUP_EVENT_COUNT} approved events`);
  }
  for (const optionId of SELECTION_OPTIONS) {
    if (optionCounts[optionId] !== PE_EVENTS_PER_OPTION_PER_GROUP || evidence.peCountsByGroupAndOption?.[groupId]?.[optionId] !== PE_EVENTS_PER_OPTION_PER_GROUP) {
      throw new Error(`group ${groupId} ${optionId} must contain exactly ${PE_EVENTS_PER_OPTION_PER_GROUP} PE events`);
    }
  }
  const otherSelectionEvents = groupEvents.filter((event) =>
    event.selection != null && event.selection.selectionGroupId !== SELECTION_GROUP_ID
  );
  if (otherSelectionEvents.length !== 0) {
    throw new Error(`group ${groupId} contains unexpected selection groups`);
  }
}
...
const versions = GROUPS.map((groupId) => ({
  ...
  eventCount: APPROVED_GROUP_EVENT_COUNT
}));
const parsingResult = {
...
console.log(JSON.stringify({
  ...
  groupEventCounts,
  selectionGroupId: SELECTION_GROUP_ID,
  selectionOptions: SELECTION_OPTIONS,
  groupSelectionCounts,
  failClosedEventCountPerGroup: BASE_EVENTS_PER_GROUP,
  personalizedEventCountPerGroup: PERSONALIZED_EVENTS_PER_GROUP
}, null, 2));
...
for (const name of [
  ...
  'applyCalendarPreferencesToEvents',
...
try {
  ...
  for (const version of versions) {
    ...
    const failClosedEvents = core.applyCalendarPreferencesToEvents(published.events, {});
    if (failClosedEvents.length !== BASE_EVENTS_PER_GROUP || selectionEvents(failClosedEvents).length !== 0) {
      throw new Error(`group ${version.groupId} fail-closed selection filtering verification failed`);
    }

    for (const optionId of SELECTION_OPTIONS) {
      const personalizedEvents = core.applyCalendarPreferencesToEvents(published.events, {
        electiveChoices: { [SELECTION_GROUP_ID]: optionId }
      });
      if (personalizedEvents.length !== PERSONALIZED_EVENTS_PER_GROUP) {
        throw new Error(`group ${version.groupId} ${optionId} personalized event count verification failed`);
      }
      const chosenSelectionEvents = selectionEvents(personalizedEvents);
      if (chosenSelectionEvents.length !== PE_EVENTS_PER_OPTION_PER_GROUP || chosenSelectionEvents.some((event) => event.selection.selectionOptionId !== optionId)) {
        throw new Error(`group ${version.groupId} ${optionId} selection filtering verification failed`);
      }
      ...
      if (countVevents(ics) !== PERSONALIZED_EVENTS_PER_GROUP) {
        throw new Error(`group ${version.groupId} ${optionId} personalized ICS VEVENT count verification failed`);
      }
      if (personalizedEvents.some((event) => event.assessment) && !ics.includes('DESCRIPTION:')) {
        throw new Error(`group ${version.groupId} ${optionId} assessment metadata is missing from rendered ICS`);
      }
    }
  }
  ...
  console.log(JSON.stringify({
    result: 'PRODUCTION_SCHEDULES_PUBLISHED_AND_PERSONALIZATION_VERIFIED',
