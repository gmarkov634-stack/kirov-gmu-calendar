#!/usr/bin/env python3
"""Mechanical XLSX source probe for KGMU medicine groups 401-416.

This helper intentionally performs no semantic parsing. It discovers the current
official 4-course XLSX link from the KGMU medicine timetable page, then records
checksum, workbook geometry, merged ranges, non-empty cell values and hyperlink
targets for operator/ChatGPT review under canonical G/C rules.
"""
import hashlib
import html
import io
...
def discover_source() -> dict:
    ...
    if len(matches) != 1:
        raise SystemExit(f"expected exactly one current XLSX for 401-416, found {matches}")
    return {"stream": "401-416", "url": matches[0]}
