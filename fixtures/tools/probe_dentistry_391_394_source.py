#!/usr/bin/env python3
"""Mechanical XLSX source probe for KGMU Dentistry course 3, groups 391-394."""
import hashlib
import html
import io
import json
...
OUT = ROOT / "fixtures/2026-2027-semester-1/dentistry-391-394.source-probe.json"
TIMETABLE_PAGE = "https://kirovgma.ru/raspisanie-stomatologicheskiy-fakultet"
ALLOWED_PREFIX = "/sites/default/files/files/"
SOURCE_PATTERN = re.compile(r"3_stomat[^\"'<>\s]*\.xlsx", re.IGNORECASE)
...
def discover_source() -> dict:
    ...
    if len(matches) != 1:
        raise SystemExit(f"expected exactly one current XLSX for dentistry 391-394, found {matches}")
    return {"program": "dentistry", "course": 3, "groups": EXPECTED_GROUPS, "url": matches[0]}
