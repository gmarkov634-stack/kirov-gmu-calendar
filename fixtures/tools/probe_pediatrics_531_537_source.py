#!/usr/bin/env python3
"""Mechanical XLSX source probe for KGMU Pediatrics course 5, groups 531-537."""
import hashlib
import html
import io
import json
...
OUT = ROOT / "fixtures/2026-2027-semester-1/pediatrics-531-537.source-probe.json"
TIMETABLE_PAGE = "https://kirovgma.ru/raspisanie-pediatricheskiy-fakultet"
ALLOWED_PREFIX = "/sites/default/files/files/"
SOURCE_PATTERN = re.compile(r"5_ped[^\"'<>\s]*\.xlsx", re.IGNORECASE)
...
def discover_source() -> dict:
    ...
    if len(matches) != 1:
        raise SystemExit(f"expected exactly one current XLSX for pediatrics 531-537, found {matches}")
    return {
        "program": "pediatrics",
        "course": 5,
        "groups": [str(value) for value in range(531, 538)],
        "url": matches[0],
    }
