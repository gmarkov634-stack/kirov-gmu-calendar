...
def workbook_dump(source: dict) -> dict:
    ...
    for sheet in workbook.worksheets:
        ...
        sheets.append({"title": sheet.title, "maxRow": sheet.max_row, "maxColumn": sheet.max_column, "mergedRanges": [str(rng) for rng in sheet.merged_cells.ranges], "nonEmptyCellCount": len(non_empty), "nonEmptyCells": non_empty})
    return {**source, "sha256": hashlib.sha256(data).hexdigest(), "byteLength": len(data), "sheetNames": workbook.sheetnames, "sheets": sheets}


def main() -> None:
    payload = {"schema": "kgmu-mechanical-source-probe-v1", "semanticParsingPerformed": False, "timetablePage": TIMETABLE_PAGE, "source": workbook_dump(discover_source())}
    OUT.parent.mkdir(parents=True, exist_ok=True)
    OUT.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    source = payload["source"]
    print(json.dumps({"program": source["program"], "course": source["course"], "groups": source["groups"], "url": source["url"], "sha256": source["sha256"], "byteLength": source["byteLength"], "sheetNames": source["sheetNames"], "dimensions": [[s["title"], s["maxRow"], s["maxColumn"], len(s["mergedRanges"]), s["nonEmptyCellCount"]] for s in source["sheets"]]}, ensure_ascii=False, indent=2))
