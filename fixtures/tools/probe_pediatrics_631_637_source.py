#!/usr/bin/env python3
"""Mechanical XLSX source probe for KGMU Pediatrics course 6, groups 631-637.
...
OUT = ROOT / "pediatrics-631-637-source-probe.json"
XLSX_OUT = ROOT / "pediatrics-631-637-source.xlsx"
TIMETABLE_PAGE = "https://kirovgma.ru/raspisanie-pediatricheskiy-fakultet"
ALLOWED_PREFIX = "/sites/default/files/files/"
SOURCE_PATTERN = re.compile(r"6_ped[^\"'<>\s]*\.xlsx", re.IGNORECASE)
...
def discover_source() -> dict:
    ...
    if len(matches) != 1:
        raise SystemExit(f"expected exactly one current XLSX for pediatrics 631-637, found {matches}")
    return {"program": "pediatrics", "course": 6, "groups": [str(value) for value in range(631, 638)], "url": matches[0]}
