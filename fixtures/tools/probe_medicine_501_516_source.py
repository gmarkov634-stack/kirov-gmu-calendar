#!/usr/bin/env python3
"""Mechanical XLSX source probe for KGMU medicine groups 501-516.
...
workbook geometry, merged ranges, non-empty cell values and hyperlink targets for
operator/ChatGPT review under canonical G/R/C/S rules.
"""
import hashlib
import html
import io
...
def discover_source() -> dict:
    ...
    if len(matches) != 1:
        raise SystemExit(f"expected exactly one current XLSX for medicine 501-516, found {matches}")
    return {"stream": "501-516", "url": matches[0]}
