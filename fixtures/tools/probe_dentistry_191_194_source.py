#!/usr/bin/env python3
"""Mechanical XLSX source probe for KGMU Dentistry course 1, groups 191-194.
...
OUT = ROOT / "dentistry-191-194-source-probe.json"
XLSX_OUT = ROOT / "dentistry-191-194-source.xlsx"
TIMETABLE_PAGE = "https://kirovgma.ru/raspisanie-stomatologicheskiy-fakultet"
ALLOWED_PREFIX = "/sites/default/files/files/"
SOURCE_PATTERN = re.compile(r"1_stomat[^\"'<>\s]*\.xlsx", re.IGNORECASE)
...
def discover_source() -> dict:
    ...
    if len(matches) != 1:
        raise SystemExit(f"expected exactly one current XLSX for dentistry 191-194, found {matches}")
    return {"program": "dentistry", "course": 1, "groups": ["191", "192", "193", "194"], "url": matches[0]}
