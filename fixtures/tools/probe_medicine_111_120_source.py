#!/usr/bin/env python3
"""Mechanical XLSX source probe for KGMU medicine groups 111-120.
...
OUT = ROOT / "medicine-111-120-source-probe.json"
TIMETABLE_PAGE = "https://kirovgma.ru/lechebnyy-fakultet-raspisanie"
ALLOWED_PREFIX = "/sites/default/files/files/"
SOURCE_PATTERN = re.compile(r"1_lech\._2_potok[^\"'<>\s]*\.xlsx", re.IGNORECASE)
...
def discover_source() -> dict:
    ...
    if len(matches) != 1:
        raise SystemExit(f"expected exactly one current XLSX for 111-120, found {matches}")
    return {"stream": "111-120", "url": matches[0]}
