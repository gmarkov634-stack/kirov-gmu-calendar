#!/usr/bin/env python3
"""Mechanical XLSX source probe for KGMU Pediatrics course 2, groups 231-239.
...
def discover_source() -> dict:
    ...
    if len(matches) != 1:
        raise SystemExit(f"expected exactly one current XLSX for pediatrics 231-239, found {matches}")
    return {"program": "pediatrics", "course": 2, "groups": [str(value) for value in range(231, 240)], "url": matches[0]}
