...
def main():
    ...
    # Month name in row 10 establishes the month for each calendar column.
    month_by_col = {}
    for rng in ranges:
    ...
    for row in range(13, 29):
        ...
        blocks = []
        for rng in ranges:
            if rng.min_row <= row <= rng.max_row and rng.min_col >= 3 and rng.max_col >= 3:
                # Only horizontal source blocks rooted on this group row.
    ...
    print(json.dumps({
        ...
        "referenceRows": [item["row"] for item in reference_rows],
