...
export function parseForeignRWorkbook(workbook,{university='kgmu',program='foreign',course=1,academicYear=null,semester=null}={}){
  ...
  const qa={status:uncovered.length||extraLessonFailures.length?'REVIEW_REQUIRED':'PASS',sourceAnchorCount:anchors.length,coveredSourceAnchors:anchors.length-uncovered.length,uncovered,extraLessonExpectations:expectations,extraLessonFailures,sourceConflicts,eventCount:final.length,eventCountsByGroup:Object.fromEntries(schedules.map(s=>[s.group.code,s.events.length])),inferredWeekdayRows:[...weekdays.entries()].filter(([row])=>![...(sheet.cells||[])].some(c=>c.col===1&&c.row===row&&WEEKDAYS.has(clean(c.value).toLowerCase()))).map(([row,weekday])=>({row,weekday}))};
  return {schedules,qa};
}
