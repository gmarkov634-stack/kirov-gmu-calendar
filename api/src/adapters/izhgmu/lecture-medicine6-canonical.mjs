...
function candidate({parsed,metadata,source,parserName}){const n=normalizeInputs({parsed,metadata,source});const events=[];for(const series of parsed.courseWideCoreSeries||[]){if(series.status!=='ok'||!series.groups?.includes(n.metadata.group)||!series.startTime||!series.endTime)continue;for(const date of [...new Set(series.dates||[])])events.push(eventForDate({series,date,metadata:n.metadata,source:n.source}));}events.sort((a,b)=>`${a.timing.date}T${a.timing.start_time}`.localeCompare(`${b.timing.date}T${b.timing.start_time}`));return{schema_version:'1.0',schedule:{university_code:'izhgmu',academic_year:n.metadata.academicYear,semester:n.metadata.semester,faculty_code:n.metadata.facultyCode,course:n.metadata.course,group:n.metadata.group,period:{start_date:parsed.period.start_date,end_date:parsed.period.end_date,week1_start_date:parsed.period.week1_start_date},source_files:[n.source.fileName],generated_at:null,parser:parserName,schedule_version_id:null,previous_schedule_version_id:null,content_fingerprint:null,version_created_at:null},events};}
