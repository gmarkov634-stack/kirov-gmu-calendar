...
function compactResult(result) {
  ...
  return {
    ...
    qa: result.qa ? {
      ...
      groupCount: result.qa.groupCount ?? null,
      eventCount: result.qa.eventCount ?? null,
      groups: result.qa.groups ?? [],
    } : null,
    published: result.published ? {
      ...
      publications: Array.isArray(result.published.publications)
        ? result.published.publications.map((item) => ({
            group: item.group ?? null,
            scheduleVersionId: item.scheduleVersionId ?? null,
            previousScheduleVersionId: item.previousScheduleVersionId ?? null,
            diffCounts: item.diff?.counts ?? null,
            unchanged: item.publication?.unchanged ?? false,
          }))
        : [],
    } : null,
  };
...
export function createScheduleReviewControlHandler({ reviewedService, fetchImpl = globalThis.fetch, verifyOidcToken, nowFactory = Date.now }) {
  ...
  return async function handleScheduleReviewControl(request, response) {
    ...
    if (!reviewedService || typeof reviewedService.submitCanonical !== "function" || typeof reviewedService.publishReview !== "function") {
      return sendJson(response, 503, { error: "schedule_review_control_not_configured" });
    }
