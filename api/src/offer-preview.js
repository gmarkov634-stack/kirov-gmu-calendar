...
function validOfferPeriod(config) {
  ...
  if (!normalizeAcademicYear(academicYear) || ![1, 2].includes(semester)) return null;
...
export function createOfferPreviewHandler({ store, config, now = () => new Date() }) {
  return async function offerPreviewHandler(request, response) {
    allowCors(request, response, config);
    if (request.method === "OPTIONS") return send(response, 204, {});
    if (request.method !== "GET") return send(response, 405, { error: "method_not_allowed" });

    const url = new URL(request.url, "http://localhost");
    const match = url.pathname.match(/^\/api\/v2\/catalog\/([^/]+)\/([^/]+)\/(\d+)\/([^/]+)\/preview$/);
    ...
    const requestedGroupCode = decodeURIComponent(match[4]);
    if (!UNIVERSITY_ID.test(university) || !PROGRAM_ID.test(program) || !Number.isInteger(course) || course < 1 || course > 9 || !requestedGroupCode) {
    ...
    try {
      ...
      console.error("offer preview unavailable", error);
      return send(response, 503, { error: "preview_unavailable" });
    }
  };
}
