...
function fixture() {
  ...
  const merge = (row1, col1, row2, col2) => merges.push({
    ...
    startRef: ref(col1, row1),
    endRef: ref(col2, row2),
    startRow: row1,
    endRow: row2,
    startCol: col1,
    endCol: col2,
  ...
  for (const [group, groupRuns] of Object.entries(runs)) {
    ...
    for (const [start, end, subject] of groupRuns) {
      for (let c = start; c <= end; c += 1) {
        styledCells.push({ row, col: c, ref: ref(c, row), value: anchors[group][c] || "", styleId: fill[subject] + 10, fillId: fill[subject] });
      }
    }
  ...
  add(23, 3, "Academic discipline");
  add(23, 21, "Form of assessment");
  add(23, 27, "Department");
  add(23, 42, "Place of practical training");
  add(23, 68, "Address");
  add(23, 76, "Timing of classes");
  add(24, 76, "1st part of the day");
  add(24, 80, "2nd part of the day");
  ...
  for (const [r, discipline, assessment, department, base, address, shift1, shift2] of footer) {
    add(r, 3, discipline);
    if (assessment) add(r, 21, assessment);
    add(r, 27, department);
    add(r, 42, base);
    add(r, 68, address);
    if (shift1) add(r, 76, shift1);
    if (shift2) add(r, 80, shift2);
  }
  merge(32, 21, 33, 26);
...
test("C-FIO course 4 expands color-coded cycles without weakening fail-closed semantics", () => {
  ...
  assert.deepEqual(result.qa.mainGridSubjectDaysByGroup, {
    "401и": 79,
    "402и": 79,
    "403и": 80,
    "404и": 79,
  ...
  const neuro401 = events
    .filter((event) => event.group === "401и" && event.title === "Неврология, нейрохирургия")
