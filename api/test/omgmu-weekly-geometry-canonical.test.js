...
import { buildWeeklyGridCanonicalBatch } from "../src/adapters/omgmu/weekly-grid.mjs";
...
test("keeps the two O65 histology source-series independent before final user-event merge", () => {
...
test("passes a geometry-backed weekly subset through the shared canonical pipeline without O65 merging", () => {
  const batch = buildWeeklyGridCanonicalBatch(histologyOnlyGeometry(), { metadata: metadata("1109"), source });
  assert.equal(batch.events.length, 19);
  ...
  assert.deepEqual(july20.map((event) => [event.timing.start_time, event.timing.end_time]), [
    ["13:30", "15:55"],
    ["16:00", "18:25"],
  ]);
  ...
  assert.match(prepared.ics, /DTSTART:20260720T160000/);
