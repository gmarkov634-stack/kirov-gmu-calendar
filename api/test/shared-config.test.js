...
test("IZHGMU source page is configurable independently from its future landing", () => {
  const configured = loadConfig({ IZHGMU_SCHEDULE_PAGE: "https://example.test/izh-schedule" });
  assert.equal(configured.izhgmuSchedulePage, "https://example.test/izh-schedule");
  assert.equal(configured.universitySiteUrls.izhgmu, "");

  const defaults = loadConfig({});
  assert.equal(defaults.izhgmuSchedulePage, "https://igma.ru/component/content/article/647-raspisanie?Itemid=108&catid=132");
});
