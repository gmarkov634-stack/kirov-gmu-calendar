...
async function readRoot(name) { return fs.readFile(path.join(root, name), "utf8"); }
...
test("group selection uses preview and the required server-scoped routes", async () => {
  const source = await readRoot("app.js");
  assert.ok(source.includes('setStep("preview")'));
  for (const required of ["/api/v2/meta", "/api/v2/trials", "/api/v2/trials/continue/", "/preview", "conversionId", "trial_to_paid"]) {
    assert.ok(source.includes(required), `missing frontend invariant: ${required}`);
  }
  assert.ok(source.includes('runtimeMeta.trials === "open"'));
  assert.ok(source.includes('runtimeMeta.sales === "open"') || source.includes('runtimeMeta.sales !== "open"'));
});

test("static landing remains fail-closed before runtime gates load", async () => {
