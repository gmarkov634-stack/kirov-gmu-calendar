...
function makeEvent({ date = "2026-09-01", start = "09:00", end = "10:30", discipline = "Педиатрия", type = "practice", room = "305", eventId = null, sourceRange = "D18:H18" } = {}) {
  return {
    schema_version: "1.0",
    system: { event_id: eventId, schedule_version_id: null, fingerprint: null, revision: null, created_at: null, updated_at: null },
...
const T3 = "2026-08-13T11:00:00.000Z";

test("first import assigns event IDs, fingerprints, revision metadata and one version ID", () => {
  counter = 0;
  versionCounter = 0;
  const { batch, diff } = versionSchedule(null, makeBatch([makeEvent(), makeEvent({ date: "2026-09-08" })]), { eventIdFactory: idFactory, versionIdFactory: versionFactory, now: T1 });
  ...
  assert.equal(batch.events[0].system.revision, 1);
  assert.equal(batch.events[0].system.created_at, T1);
  assert.equal(batch.events[0].system.updated_at, T1);
  assert.equal(batch.schedule.version_created_at, T1);
...
test("same event with changed time preserves event_id and increments revision", () => {
  counter = 0;
  versionCounter = 0;
  const first = versionSchedule(null, makeBatch([makeEvent()]), { eventIdFactory: idFactory, versionIdFactory: versionFactory, now: T1 }).batch;
  const second = versionSchedule(first, makeBatch([makeEvent({ start: "10:40", end: "12:10" })]), { eventIdFactory: idFactory, versionIdFactory: versionFactory, now: T2 });
  ...
  assert.equal(second.batch.events[0].system.created_at, T1);
  assert.equal(second.batch.events[0].system.updated_at, T2);
...
test("identical re-import remains on current revision and preserves timestamps", () => {
  counter = 0;
  versionCounter = 0;
  const first = versionSchedule(null, makeBatch([makeEvent()]), { eventIdFactory: idFactory, versionIdFactory: versionFactory, now: T1 }).batch;
  const second = versionSchedule(first, makeBatch([makeEvent()]), { eventIdFactory: idFactory, versionIdFactory: versionFactory, now: T2 });
  ...
  assert.equal(second.batch.schedule.version_created_at, T1);
  assert.equal(second.batch.events[0].system.revision, 1);
  assert.equal(second.batch.events[0].system.updated_at, T1);
});

test("source anchor preserves id when date changes", () => {
  counter = 0;
  versionCounter = 0;
  const first = versionSchedule(null, makeBatch([makeEvent({ date: "2026-09-01", sourceRange: "D18:H18" })]), { eventIdFactory: idFactory, versionIdFactory: versionFactory, now: T1 }).batch;
  const second = versionSchedule(first, makeBatch([makeEvent({ date: "2026-09-02", sourceRange: "D18:H18" })]), { eventIdFactory: idFactory, versionIdFactory: versionFactory, now: T2 });
  assert.equal(second.batch.events[0].system.event_id, first.events[0].system.event_id);
  assert.equal(second.batch.events[0].system.revision, 2);
...
test("added and removed events are classified separately", () => {
  counter = 0;
  versionCounter = 0;
  const first = versionSchedule(null, makeBatch([makeEvent({ discipline: "Педиатрия" }), makeEvent({ discipline: "Биохимия", sourceRange: "I18:L18" })]), { eventIdFactory: idFactory, versionIdFactory: versionFactory, now: T1 }).batch;
  const second = versionSchedule(first, makeBatch([makeEvent({ discipline: "Педиатрия" }), makeEvent({ discipline: "Фармакология", sourceRange: "M18:P18" })]), { eventIdFactory: idFactory, versionIdFactory: versionFactory, now: T2 });
  ...
  assert.equal(second.diff.added[0].revision, 1);
...
test("recurring ambiguous events are not fuzzily cross-matched", () => {
  ...
  ]), { eventIdFactory: idFactory, versionIdFactory: versionFactory, now: T1 }).batch;
  const incoming = makeBatch([
    makeEvent({ date: "2026-09-02", sourceRange: "X1" }),
    makeEvent({ date: "2026-09-09", sourceRange: "X2" }),
  ]);
  const second = versionSchedule(first, incoming, { eventIdFactory: idFactory, versionIdFactory: versionFactory, now: T2 });
...
test("incoming existing event_id has highest matching priority", () => {
  counter = 0;
  versionCounter = 0;
  const first = versionSchedule(null, makeBatch([makeEvent({ eventId: "evt_fixed" })]), { eventIdFactory: idFactory, versionIdFactory: versionFactory, now: T1 }).batch;
  const incoming = makeEvent({ date: "2026-10-01", sourceRange: "Z99", eventId: "evt_fixed" });
  const second = versionSchedule(first, makeBatch([incoming]), { eventIdFactory: idFactory, versionIdFactory: versionFactory, now: T2 });
  assert.equal(second.diff.counts.changed, 1);
...
test("A to B to A creates new revisions and increments event sequence each time", () => {
  counter = 0;
  versionCounter = 0;
  const a1 = versionSchedule(null, makeBatch([makeEvent({ start: "09:00" })]), { eventIdFactory: idFactory, versionIdFactory: versionFactory, now: T1 }).batch;
  const b = versionSchedule(a1, makeBatch([makeEvent({ start: "10:40" })]), { eventIdFactory: idFactory, versionIdFactory: versionFactory, now: T2 }).batch;
  const a2 = versionSchedule(b, makeBatch([makeEvent({ start: "09:00" })]), { eventIdFactory: idFactory, versionIdFactory: versionFactory, now: T3 }).batch;
  ...
  assert.equal(a1.events[0].system.revision, 1);
  assert.equal(b.events[0].system.revision, 2);
  assert.equal(a2.events[0].system.revision, 3);
  assert.equal(a2.events[0].system.created_at, T1);
  assert.equal(a2.events[0].system.updated_at, T3);
});
