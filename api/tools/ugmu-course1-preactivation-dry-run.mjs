...
const EXPECTED_TOTAL_EVENTS = 17301;
const DEFAULT_GENERATED_AT = "2026-08-22T08:30:00.000Z";
...
function rollbackSnapshotKey(storageKey, manifestId) {
  ...
  return `rollback/ugmu/course1-preactivation/${manifestId}/${encoded}`;
}

const packageDir = path.resolve(arg("package-dir", "data/imports/ugmu-course1/canonical-package"));
const regressionPath = path.resolve(arg("regression-report", "data/imports/ugmu-course1/versioning/course1-versioning-regression.json"));
const outputDir = path.resolve(arg("output-dir", "data/imports/ugmu-course1/preactivation-dry-run"));
...
if (!packageManifest.passed || packageManifest.publicationAllowed !== false || packageManifest.groups !== 50 || packageManifest.events !== EXPECTED_TOTAL_EVENTS) {
  throw new Error("UGMU course-1 canonical package is not approved fail-closed evidence");
}
if (!regression.passed || regression.publicationAllowed !== false || regression.totals?.groups !== 50 || regression.totals?.events !== EXPECTED_TOTAL_EVENTS) {
  throw new Error("UGMU course-1 versioning regression is not approved fail-closed evidence");
}

const packageManifestSha256 = sha256(packageManifestText);
const manifestId = `ugmu-course1-2026-autumn-${packageManifestSha256.slice(0, 12)}`;
...
for (const group of EXPECTED_GROUPS) {
  ...
  const expectedGroupId = `ugmu:medicine:1:stream-${stream}:${group}`;
  ...
  const pointer = {
    version: 1,
    kind: "ugmu-course1-preactivation-dry-run-pointer",
    manifestId,
    university: "ugmu",
...
const manifest = {
  ...
  checks: {
    exactGroupSet: groups.length === 50 && groups.every((item, index) => item.group === EXPECTED_GROUPS[index]),
    exactEventTotal: totalEvents === EXPECTED_TOTAL_EVENTS,
    exactStreamGroups: streamTotals["1"].groups === 12 && streamTotals["2"].groups === 12 && streamTotals["3"].groups === 12 && streamTotals["4"].groups === 14,
    exactStreamEvents: streamTotals["1"].events === 4286 && streamTotals["2"].events === 4263 && streamTotals["3"].events === 4026 && streamTotals["4"].events === 4726,
    uniqueStorageKeys: new Set(groups.map((item) => item.storageKey)).size === 50,
    uniqueRollbackPointers: new Set(groups.map((item) => item.rollback.snapshotKey)).size === 50,
    ...
    allRollbackKeysUgmuNamespaced: groups.every((item) => item.rollback.snapshotKey.startsWith(`rollback/ugmu/course1-preactivation/${manifestId}/`)),
  },
  activationAuthority: {
    granted: false,
    stagingWriteAuthority: false,
    salesAuthority: false,
    reason: "Deterministic course-1 dry-run only; production storage staging requires a separate fail-closed authority and rollback gate.",
  },
  publicationAllowed: false,
  nextRequiredBoundary: "course1-production-storage-staging-review",
};
...
console.log(`UGMU course-1 preactivation dry-run: ${manifest.passed ? "PASS" : "FAIL"}`);
