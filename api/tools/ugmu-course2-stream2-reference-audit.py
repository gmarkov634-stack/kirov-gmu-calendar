...
EXPECTED_RAW_PATTERN_COUNT = 206
...
def candidate_for(source_title: str, reference_titles: list[str]) -> dict[str, Any]:
    ...
    if len(normalized) == 1:
        return {
            "status": "normalized-exact-candidate",
            "candidates": normalized,
            "mappingApplied": False,
        }
    if len(normalized) > 1:
        return {
            "status": "ambiguous-normalized",
            "candidates": normalized,
            "mappingApplied": False,
        }
    ...
    if len(prefix) == 1:
        return {
            "status": "unique-prefix-candidate",
            "candidates": prefix,
            "mappingApplied": False,
        }
    if len(prefix) > 1:
        return {
            "status": "ambiguous-prefix",
            "candidates": prefix,
            "mappingApplied": False,
        }
    return {
...
def build(raw_path: Path, pdf_path: Path) -> dict[str, Any]:
    ...
    if raw.get("course") != 2 or raw.get("stream") != 2:
        raise RuntimeError("Reference audit is restricted to UGMU medicine course 2 stream II")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="UGMU course-2 stream-II reference audit; no mappings are applied")
