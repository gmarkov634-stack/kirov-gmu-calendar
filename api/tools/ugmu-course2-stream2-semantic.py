...
# User-approved decisions for the exact stream-II source SHA above.
# semanticTitle is the title retained for downstream semantics.
# referenceDiscipline is the literal page-2 row used only for department/address evidence.
APPROVED_TITLE_DECISIONS: dict[str, dict[str, str | None]] = {
    ...
    "Клиническая": {
        "semanticTitle": "Клиническая биохимия",
        "referenceDiscipline": "Клиническая биохимия",
        "resolution": "user-approved-reference-title",
    },
    "Микробиология, вирусология иммунология": {
        "semanticTitle": "Микробиология, вирусология, иммунология",
        "referenceDiscipline": "Микробиология, вирусология, иммунология",
        "resolution": "user-approved-reference-title",
    },
    "Графическая визуализация научных данных": {
        "semanticTitle": "Графическая визуализация научных данных",
        "referenceDiscipline": "Графическа я визуализация научных данных",
        "resolution": "user-approved-reference-link-only",
    },
    "Сохранение психического здоровья: механизмы адаптации и управления стрессом": {
        "semanticTitle": "Сохранение психического здоровья: механизмы адаптации и управления стрессом",
        "referenceDiscipline": "Сохранение психического здоровья: механизмы адаптации и",
        "resolution": "user-approved-reference-link-only",
    },
...
EXPECTED_RESOLUTION_COUNTS = {
    "literal-exact": 154,
    "user-approved-reference-title": 42,
    "user-approved-reference-link-only": 2,
    "user-approved-no-reference": 8,
}
EXPECTED_TYPE_COUNTS = {"other": 110, "lecture": 96}
...
def extract_reference_rows(pdf_path: Path) -> list[dict[str, str]]:
    ...
    with pdfplumber.open(pdf_path) as document:
        if len(document.pages) < 2:
            raise RuntimeError("UGMU stream-II source has no reference page")
    ...
    if not rows:
        raise RuntimeError("UGMU stream-II discipline reference table not found")
...
def resolve_title(source_title: str, references: dict[str, dict[str, str]]) -> dict[str, Any]:
    ...
    if not decision:
        raise RuntimeError(f"No approved stream-II title decision for: {source_title}")
    ...
    if not row:
        raise RuntimeError(
            f"Approved reference row disappeared for {source_title}: {reference_discipline}; manual review required"
        )
    return {
        "semanticTitle": decision["semanticTitle"],
...
def build(raw_path: Path, pdf_path: Path) -> dict[str, Any]:
    ...
    if raw.get("course") != 2 or raw.get("stream") != 2:
        raise RuntimeError("Semantic reconciliation is restricted to UGMU medicine course 2 stream II")
    if raw.get("source", {}).get("sha256") != EXPECTED_SHA256:
        raise RuntimeError("Raw source SHA-256 does not match approved stream-II source")
    if raw.get("source", {}).get("url") != EXPECTED_SOURCE_URL:
        raise RuntimeError("Raw source URL does not match approved stream-II source")
    ...
    for required_reference in (
        "Графическа я визуализация научных данных",
        "Сохранение психического здоровья: механизмы адаптации и",
    ):
        if required_reference not in references:
            raise RuntimeError(f"Approved partial reference row changed: {required_reference}; manual review required")

    if "Лекарственные растения и основы фармакогнозии." in references:
        raise RuntimeError("Previously missing reference row now exists; new semantic review required")
    ...
    for group in EXPECTED_GROUPS:
        ...
        for pattern in raw["groups"][group]:
            ...
            if (
                resolved["semanticTitle"] != source_title
                or resolved["referenceDiscipline"] != source_title
                or resolved["titleResolution"] != "literal-exact"
            ):
                reconciliation_counts[(
                    source_title,
                    resolved["semanticTitle"],
                    resolved["referenceDiscipline"],
                    resolved["titleResolution"],
                )] += 1
    ...
    reconciliations = [
        ...
        for (source, semantic, reference, resolution), count in sorted(
            reconciliation_counts.items(), key=lambda item: (-item[1], item[0][0])
        )
    ]
    ...
    return {
        ...
        "summary": {
            ...
            "referenceLinkOnlyPatterns": resolution_counts["user-approved-reference-link-only"],
...
def main() -> None:
    parser = argparse.ArgumentParser(description="UGMU course-2 stream-II user-approved semantic reconciliation only")
