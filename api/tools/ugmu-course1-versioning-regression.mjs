...
function findUniqueMutableEvent(events) {
  ...
  if (index < 0) throw new Error("No unique UGMU course-1 event available for versioning regression");
  return index;
}
...
function postprocessed(versioned) {
  ...
  if (!qa.publishable) throw new Error(`Postprocessed UGMU course-1 version failed QA: ${JSON.stringify(qa.errors)}`);
...
const outputPath = path.resolve(arg("output", "data/imports/ugmu-course1/versioning/course1-versioning-regression.json"));
const manifest = JSON.parse(await fs.readFile(path.join(packageDir, "manifest.json"), "utf8"));
if (!manifest.passed || manifest.publicationAllowed !== false || manifest.groups !== 50 || manifest.events !== EXPECTED_TOTAL_EVENTS) {
...
for (const group of EXPECTED_GROUPS) {
  ...
  assert(canonical.schedule?.course === 1, "canonical course mismatch", errors);
  assert(canonical.events?.length > 0, "canonical event set is empty", errors);

  const v1 = prepareVersion(structuredClone(canonical), null, "2026-08-22T08:10:00.000Z", versionId(group, "v1"));
  const p1 = postprocessed(v1.batch);

  const exact = prepareVersion(structuredClone(canonical), v1.batch, "2026-08-22T08:15:00.000Z", versionId(group, "unused"));
...
const passed = groupReports.every((item) => item.passed)
  ...
  && totals.simulatedChanged === 50
  && totals.simulatedUnchanged === EXPECTED_TOTAL_EVENTS - 50
  && totals.stableUidGroups === 50
  && totals.sequenceAdvancedGroups === 50;
...
const report = {
  ...
  nextGate: passed ? "course1-preactivation-dry-run" : "fix-course1-versioning",
};

await fs.mkdir(path.dirname(outputPath), { recursive: true });
await fs.writeFile(outputPath, `${JSON.stringify(report, null, 2)}\n`);
console.log(`UGMU course-1 versioning regression: ${passed ? "PASS" : "FAIL"}`);
console.log(`Stable UID groups: ${totals.stableUidGroups}/50; SEQUENCE advanced: ${totals.sequenceAdvancedGroups}/50`);
