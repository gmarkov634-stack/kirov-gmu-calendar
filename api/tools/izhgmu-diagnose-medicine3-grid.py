...
for r in range(11,sh.nrows):
    ...
    for c in range(sh.ncols):
        ...
        if text: vals.append({'col':c+1,'value':text})
    rows.append({'row':r+1,'cells':vals})
merges=[{'r1':rlo+1,'r2':rhi,'c1':clo+1,'c2':chi} for rlo,rhi,clo,chi in sh.merged_cells if rhi>=12]
out={'version':1,'source':src,'sheet':sh.name,'rows':rows,'merges':merges}
(root/'medicine3-grid-diagnostic.json').write_text(json.dumps(out,ensure_ascii=False,indent=2)+'\n')
for row in rows: print('GRID_ROW',row['row'],json.dumps(row['cells'],ensure_ascii=False))
