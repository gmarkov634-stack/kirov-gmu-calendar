...
APPROVED_TITLE_DECISIONS: dict[str, dict[str, str]] = {
    "Биохими": {
    ...
    "Философия культурология": {
        "semanticTitle": "Философия, культурология",
        "referenceDiscipline": "Философия, культурология",
        "resolution": "user-approved-reference-title",
    },
    ...
    # Keep the page-1 title; use the truncated page-2 row only for metadata.
    "Научно-исследовательская работа": {
        "semanticTitle": "Научно-исследовательская работа",
        "referenceDiscipline": "Научно-исследовательская работа (получение первичных",
        "resolution": "user-approved-reference-link-only",
    },
    # Keep the correctly spelled page-1 title; link metadata to the typoed page-2 row.
    "Современная научная картина мира.": {
        "semanticTitle": "Современная научная картина мира.",
        "referenceDiscipline": "Современная найчная картина мира",
        "resolution": "user-approved-reference-link-only",
    },
}
...
EXPECTED_RESOLUTION_COUNTS = {
    "literal-exact": 141,
    "user-approved-reference-title": 39,
    "user-approved-reference-link-only": 24,
}
...
def extract_reference_rows(pdf_path: Path) -> list[dict[str, str]]:
    ...
    with pdfplumber.open(pdf_path) as document:
        if len(document.pages) < 2:
            raise RuntimeError("UGMU stream-IV source has no second page")
...
def resolve_title(source_title: str, references: dict[str, dict[str, str]]) -> dict[str, Any]:
    ...
    if not decision:
        raise RuntimeError(f"No approved stream-IV title decision for: {source_title}")
...
def build(raw_path: Path, pdf_path: Path) -> dict[str, Any]:
    ...
    if raw.get("mode") != "raw-weekly-patterns-only" or raw.get("course") != 2 or raw.get("stream") != 4:
        raise RuntimeError("Unexpected raw payload; stream-IV review required")
    ...
    required_special_rows = {
        "Современная найчная картина мира",
        "Научно-исследовательская работа (получение первичных",
    }
    if not required_special_rows.issubset(references):
        raise RuntimeError("User-approved stream-IV reference-link rows changed; manual review required")
    if "Современная научная картина мира." in references or "Современная научная картина мира" in references:
        raise RuntimeError("Correctly spelled Modern scientific picture reference row now exists; manual review required")

    semantic_groups: dict[str, list[dict[str, Any]]] = {}
    resolution_counts: Counter[str] = Counter()
    type_counts: Counter[str] = Counter()
    reconciliation_counts: Counter[tuple[str, str, str, str]] = Counter()
    ...
    return {
        ...
        "review": {
            ...
            "sourcePeriodTypoStillUnapplied": "01.09.2026 – 23.12.2027",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="UGMU course-2 stream-IV user-approved semantic reconciliation only")
