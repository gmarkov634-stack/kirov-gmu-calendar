...
def build(raw_path: Path, pdf_path: Path) -> dict[str, Any]:
    ...
    if raw.get("course") != 2 or raw.get("stream") != 3:
        raise RuntimeError("Reference audit is restricted to UGMU medicine course 2 stream III")
    if raw.get("source", {}).get("sha256") != EXPECTED_SHA256:
        raise RuntimeError("Raw source SHA-256 does not match approved stream-III source")
    if raw.get("source", {}).get("url") != EXPECTED_SOURCE_URL:
        raise RuntimeError("Raw source URL does not match approved stream-III source")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="UGMU course-2 stream-III reference audit; no mappings are applied")
