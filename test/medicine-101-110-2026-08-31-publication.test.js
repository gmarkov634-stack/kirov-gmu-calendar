...
test('updated medicine 101-110 post-QA publication evidence pins the exact compatibility-approved candidate', async () => {
  ...
  assert.deepEqual(publication.groupFacultativeEventCounts, Object.fromEntries(groups.map((groupId) => [groupId, 92])));
  ...
  assert.equal(publication.compatibilityGateEvidence.coreBaseCommit, '58368c03d487ae7e20298f45e4b24891a54df613');
  assert.equal(publication.compatibilityGateEvidence.kgmuCandidateCommit, '462a5e73d6ba647debe1ec0b75efd9c9d1ff5d25');
  assert.equal(publication.compatibilityGateEvidence.kgmuMainMergeCommit, '2349545ed58036e9be68b57d994d82fc3c91b00b');

  assert.equal(publication.sharedContractEvidence.repository, 'gmarkov634-stack/medical-calendar-core');
  assert.equal(publication.sharedContractEvidence.commit, '58368c03d487ae7e20298f45e4b24891a54df613');
  ...
  for (const version of plan.versions) {
    assert.equal(version.versionId, `kgmu-2026-2027-s1-medicine-${version.groupId}-4834a447edd3cccf`);
  }
});

test('updated medicine 101-110 publisher is fail-closed around production replacement and subscription contracts', async () => {
  const source = await readFile(new URL('../ops/publish-medicine-101-110-2026-08-31.mjs', import.meta.url), 'utf8');
...
test('updated medicine 101-110 publisher preflight reproduces the exact transition plan without DB access', async () => {
  ...
  for (const groupId of groups) {
    assert.match(stdout, new RegExp(`kgmu-2026-2027-s1-medicine-${groupId}-1d56b5b52c6eb6b7`));
    assert.match(stdout, new RegExp(`kgmu-2026-2027-s1-medicine-${groupId}-4834a447edd3cccf`));
  }
});
