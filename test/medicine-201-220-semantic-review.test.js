...
test('medicine 201-220 semantic review is source-bound and fully resolved', async () => {
  ...
  assert.equal(first.normalizedEvents, 2661);
  ...
  assert.equal(first.operatorConfirmations[0].additionalLocator, 'G28');
  assert.equal(first.operatorConfirmations[0].explicitDate, '2026-12-30');
  assert.equal(new Date('2026-12-30T00:00:00Z').getUTCDay(), 3); // Wednesday
  assert.match(first.operatorConfirmations[0].decision, /do not synthesize 31\.12/);
  ...
  assert.equal(value.aggregate.normalizedEvents, 5307);
